...
export function OrdersLineChart({ data }: { data: DayData[] }) {
  if (data.length === 0) {
    return (
      <p className="py-12 text-center text-sm text-muted-foreground">
        Sin datos de órdenes en este período.
      </p>
    );
  }
      ...
      <AreaChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id="ordersGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="oklch(0.78 0.22 130)" stopOpacity={0.3} />
            <stop offset="95%" stopColor="oklch(0.78 0.22 130)" stopOpacity={0.02} />
          </linearGradient>
        </defs>
        ...
        <Area
          type="monotone"
          dataKey="orders"
          stroke="oklch(0.78 0.22 130)"
          strokeWidth={2.5}
          fill="url(#ordersGrad)"
          dot={{ r: 3, fill: "oklch(0.78 0.22 130)", strokeWidth: 0 }}
          activeDot={{ r: 5, fill: "oklch(0.78 0.22 130)", stroke: "#fff", strokeWidth: 2 }}
        />
      </AreaChart>
    </ResponsiveContainer>
  );
